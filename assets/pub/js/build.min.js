...
$(document).ready(()=>{cleanScheduler();});function JobStruct(jobId,totalClocks,priority){this.jobId=jobId;this.totalClocks=totalClocks;this.executed=false;this.priority=priority;}
...
chartData=[];let typeProgress=$('.typeOfProcess').val();$('.table-logs').append("<li>Iniciando execução dos Jobs. Modo de execução: "+typeProgress+"</li>");startTimeJobs=new Date().getTime();if(typeProgress==="RRS"){while(jobsToExecute.length>0){var jobNow=jobsToExecute[0];$('.table-logs').append("<li>Job "+jobNow.jobId+" executando uma parte</li>");jobPreemptivo(jobNow);}}else if(typeProgress==="SJF"){jobsToExecute.sort(compareTime);for(var i=0;i<jobsToExecute.length;i++){$('.table-logs').append("<li>Job "+jobsToExecute[i].jobId+" executando </li>");jobRoundRobin(jobsToExecute[i]);$('.table-logs').append("<li>Job "+jobsToExecute[i].jobId+" finalizou </li>");}}else if(typeProgress==="PRIORITY"){jobsToExecute.sort(comparePriority);for(var i=0;i<jobsToExecute.length;i++){$('.table-logs').append("<li>Job "+jobsToExecute[i].jobId+" executando </li>");jobRoundRobin(jobsToExecute[i]);$('.table-logs').append("<li>Job "+jobsToExecute[i].jobId+" finalizou </li>");}}else if(typeProgress==="FIFO"){for(var i=0;i<jobsToExecute.length;i++){$('.table-logs').append("<li>Job "+jobsToExecute[i].jobId+" executando </li>");jobRoundRobin(jobsToExecute[i]);$('.table-logs').append("<li>Job "+jobsToExecute[i].jobId+" finalizou </li>");}}
jobsToExecute=[];}
...
if(!jobItem.executed){jobItem.totalClocks=jobItem.totalClocks-timeSlice;let now=new Date().getTime();let startTime=(now-startTimeJobs)/1000;sleep(1000);let finishTime=(new Date().getTime()-startTimeJobs)/1000;let jobExecution=new TimeExecution(jobItem.jobId,startTime,finishTime);chartData.push(jobExecution);jobsToExecute.shift();if(jobItem.totalClocks<=0){jobItem.executed=true;$('.table-logs').append("<li>Job "+jobItem.jobId+" executou totalmente. </li>");}else{jobsToExecute.push(jobItem);}}}
...
let now=new Date().getTime();let startTime=(now-startTimeJobs)/1000;while(jobItem.totalClocks>0){console.log(jobItem.totalClocks);sleep(1000);jobItem.totalClocks=jobItem.totalClocks-timeSlice;}
...
function createChart(){if((chartData.length<=0)||(chartData==null)){toast(messages.arrayTimesEmpty);return;}
calculo();$('.table-logs').append("<li>Montando gráfico.</li>");google.charts.load("current",{packages:["timeline"]});google.charts.setOnLoadCallback(drawChart);}
...
return 1;return 0;}
function sleep(milliseconds){let now=new Date().getTime();while(new Date().getTime()<(now+milliseconds)){}}
...
function cleanScheduler(){$('#cleanScheduler').on('click',function(){toast(messages.alert.restartScheduler,messages.color.success);});}'use strict';$(document).ready(()=>{init();});const init=()=>{$('[data-toggle="tooltip"]').tooltip();}