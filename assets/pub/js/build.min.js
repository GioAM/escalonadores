...
disableForm('disable');chartData=[];let typeProgress=$('.typeOfProcess').val();$('.table-logs').append("<li class='-itemjob'><p>Tipo de Escalonamento escolhido: <span class='-numbersecondjob'>"+typeProgress+"</span></p></li>");startTimeJobs=new Date().getTime();if(typeProgress==="RRS"){while(jobsToExecute.length>0){let jobNow=jobsToExecute[0];$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobNow.jobId+"</span> executando uma parte</li>");jobPreemptivo(jobNow);}}else if(typeProgress==="SJF"){jobsToExecute.sort(compareTime);for(let i=0;i<jobsToExecute.length;i++){$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobsToExecute[i].jobId+"</span><span class='-startjob'>executando</span></li>");jobRoundRobin(jobsToExecute[i]);$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobsToExecute[i].jobId+"</span><span class='-stopjob'>finalizou</span></li>");}}else if(typeProgress==="PRIORITY"){jobsToExecute.sort(comparePriority);for(let i=0;i<jobsToExecute.length;i++){$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobsToExecute[i].jobId+"</span><span class='-startjob'>executando</span></li>");jobRoundRobin(jobsToExecute[i]);$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobsToExecute[i].jobId+"</span><span class='-stopjob'>finalizou</span></li>");}}else if(typeProgress==="FIFO"){for(let i=0;i<jobsToExecute.length;i++){$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobsToExecute[i].jobId+"</span><span class='-startjob'>executando</span></li>");jobRoundRobin(jobsToExecute[i]);$('.table-logs').append("<li class='-itemjob'>Job <span class='-numberjob'>"+jobsToExecute[i].jobId+"</span><span class='-stopjob'>finalizou</span></li>");}}
jobsToExecute=[];}
...
if(!jobItem.executed){jobItem.totalClocks=jobItem.totalClocks-timeSlice;let now=new Date().getTime();let startTime=(now-startTimeJobs)/1000;sleep();let finishTime=(new Date().getTime()-startTimeJobs)/1000;let jobExecution=new TimeExecution(jobItem.jobId,startTime,finishTime);chartData.push(jobExecution);jobsToExecute.shift();if(jobItem.totalClocks<=0){jobItem.executed=true;$('.table-logs').append("<li>Job "+jobItem.jobId+" executou totalmente. </li>");}else{jobsToExecute.push(jobItem);}}}
...
let now=new Date().getTime();let startTime=(now-startTimeJobs)/1000;while(jobItem.totalClocks>0){console.log(jobItem.totalClocks);sleep();jobItem.totalClocks=jobItem.totalClocks-timeSlice;}
let finishTime=(new Date().getTime()-startTimeJobs)/1000;let jobExecution=new TimeExecution(jobItem.jobId,startTime,finishTime);chartData.push(jobExecution);}
