...
'use strict';var id=1;var startTimeJobs;var jobsToExecute=[];var chartData=[];var timeSlice;let messages={executeJobs:'Iniciando execução dos Jobs'}
function JobStruct(jobId,totalClocks,priority){this.jobId=jobId;this.totalClocks=totalClocks;this.executed=false;this.priority=priority;}
...
var newJob=new JobStruct(id,$('#clocks').val(),$('.prioritySelect').val());jobsToExecute.push(newJob);id++;$('.table-logs').append(`
		<li class="-itemjob">
	...
	`);}
function startJobs(){toast(messages.executeJobs);if(jobsToExecute.length<=0||jobsToExecute==null){alert("Adicione Jobs para a execução!");return;}
...
var options={timeline:{singleColor:'#007bff'},};chart.draw(dataTable,options);}
