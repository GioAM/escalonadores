...
var id=1;var startTimeJobs;var allJobs=[];var jobsToExecute=[];function createQueue(){var newJob=new JobStruct(id,$('#time').val());allJobs.push(newJob);jobsToExecute.push(newJob);id++;$('.table-logs').append("<li class='-itemjob'><i class='fas fa-level-up-alt -arrowjobicon'></i>Job <span class='-numberjob'>"+newJob.jobId+"</span> adicionado a fila <i class='fas fa-minus -minusarrowicon'></i> Tempo de execução: <span class='-numbersecondjob'>"+newJob.totalTime+"</span> segundo(s) </li>");}
function startJobs(){$('.table-logs').append("<li>Iniciando execução dos Jobs.</li>");if($('.typeOfProcess').val()=="roundRobinPreemptivo"){while(jobsToExecute.length>0){var jobNow=jobsToExecute[0];console.log("Job "+jobNow.jobId+" executando - P");$('.table-logs').append("<li>Job "+jobNow.jobId+" executando </li>");jobPreemptivo(jobNow);$('.table-logs').append("<li>Job "+jobNow.jobId+" finalizou </li>");console.log("Job "+jobNow.jobId+" finalizou - P");}
